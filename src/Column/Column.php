...
class Column implements Stringable
{
    ...
    public function getPaddedValue($value){
        if( $this -> fixedLength == null && $this -> paddingType !== self::PADDING_NONE ){
            throw new COSProcessorColumnException(sprintf("Padding %s was set for the column %s, but no fixedLength was set.", $this -> paddingType, __CLASS__));
        }
    ...
    public function setPaddingType($padding_type){
        ...
        if( !in_array($padding_type, $padding_types)){
            if( $this -> label )
                throw new COSProcessorColumnException(sprintf('Invalid padding type for the column %s - choose from %s', $this -> label, implode(',', $padding_types)));
            else throw new COSProcessorColumnException(sprintf('Invalid padding type - choose from %s', implode(',', $padding_types)));
        }
